...
impl Downstairs {
     ...
     */
    fn new_work(&mut self, client_id: u8) -> BTreeSet<u64> {
     ...
     */
    fn requeue_work_bulk(&mut self, client_id: u8, work: BTreeSet<u64>) {
        if self.ds_new[client_id as usize].is_empty() {
            self.ds_new[client_id as usize] = work;
        } else {
            self.ds_new[client_id as usize].extend(work.into_iter());
        }
    }
    ...
    fn get_extent_under_repair(&self) -> Option<std::ops::RangeInclusive<u64>> {
        ...
        for cid in 0..3 {
            if let Some(eur) = self.extent_limit[cid] {
                if first.is_none() {
                    first = Some(eur as u64);
                } else {
                    // We only support one extent being repaired at a time
                    assert_eq!(Some(eur as u64), first);
                }
            }
        }
        first.map(|first| {
