            ...
            f = fr.next() => {
                ...
                match f.transpose()? {
                    ...
                    Some(Message::VersionMismatch { version }) => {
                        ...
                        up.ds_transition(
                            up_coms.client_id, DsStateData::BadVersion
                        ).await;
                        bail!(
                    ...
                    Some(Message::YouAreNowActive {
                        ...
                        if !matches_self {
                            ...
                            up.ds_transition(
                                up_coms.client_id, DsStateData::New
                            ).await;
                            if !match_gen {
                    ...
                    Some(Message::YouAreNoLongerActive {
                        ...
                        up.ds_transition(up_coms.client_id, DsStateData::New).await;
                    ...
                    Some(Message::LastFlushAck { last_flush_number }) => {
                        ...
                        assert_eq!(my_state, DsStateData::Offline);
                        info!(
                            up.log,
                    ...
                    Some(Message::UuidMismatch { expected_id }) => {
                        ...
                        up.ds_transition(
                            up_coms.client_id, DsStateData::Disabled
                        ).await;
            ...
            f = fr.next() => {
                ...
                match f.transpose()? {
                    ...
                    Some(Message::YouAreNoLongerActive {
                        ...
                        up.ds_transition(up_coms.client_id, DsStateData::Disabled).await;
                        up.set_inactive(CrucibleError::NoLongerActive).await;
                    ...
                    None => {
                        ...
                        if st == DsStateData::Active || st == DsStateData::Repair {
                            ...
                            if up.downstairs
                                ...
                                assert_eq!(st, DsStateData::Repair);
                                info!(
                                    up.log,
                        ...
                        } else if st == DsStateData::FailedRepair {
...
struct Downstairs {
    ...
    /// Indicates whether a repair task is running
    ///
    /// Normally, this can be checked by seeing if any of the downstairs client
    /// states are `LiveRepair { .. }`; however, they could fail (and be marked
    /// as `Faulted`) before the repair task finishes.
    repair_task_is_running: bool,
}
...
impl Downstairs {
    ...
    async fn enqueue(
        ...
        // Send this job to each client, potentially skipping it if the client
        // is midway through live-repair.
    ...
    fn enqueue_repair(
        ...
        for cid in ClientId::iter() {
            ...
            match current {
                ...
                | DsStateData::Replacing
                | DsStateData::LiveRepairReady => {
                    client.set_job_state(io.ds_id, IOState::Skipped);
    ...
    fn process_ds_completion(
        ...
        if job
            .io_state
            .client_state_matches(client_id, &IOState::Skipped)
        {
            ...
            warn!(
                self.log,
                "[{client_id}] Dropping already skipped job {ds_id}"
            );
            return Ok(true);
        }
        ...
        let read_data: Result<Vec<ReadResponse>, CrucibleError> =
            if let Some(context) = &encryption_context {
                if let Ok(mut responses) = responses {
                    ...
                    error!(
                        self.log,
                        "[{client_id}] DS Reports error {responses:?} on job \
                         {ds_id}, {job:?} EC",
                    );
                    // bad responses
                    responses
        ...
        let new_state = if let Err(ref e) = read_data {
            error!(
                self.log,
                "[{client_id}] Reports error {e:?} on job {ds_id}, {job:?}",
            );
        ...
        let old_state = client.set_job_state(ds_id, new_state.clone());
        ...
        if let IOState::Error(e) = new_state {
            if let Err(e) = client.handle_error(job, e) {
                panic!("unhandled error on {client_id} {job:?}: {e:?}");
            }
        } else if *self.ack_state.get(&job.ds_id).unwrap() == AckStatus::Acked {
            assert_eq!(new_state, IOState::Done);
            if let Err(e) = client.handle_acked_job(
                job,
                &read_data.unwrap(),
                &read_response_hashes,
            ) {
                panic!(
                    "failed to handle acked work for {client_id} {job:?}: {e:?}"
                );
            }
        } else {
            assert_eq!(new_state, IOState::Done);
            ...
            match r {
                ...
                Err(e) => {
                    panic!(
                        "failed to handle unacked work for \
                         {client_id} {job:?}: {e:?}"
                    );
                }
            }
...
impl DownstairsClient {
    ...
    fn in_progress(&mut self, ds_id: JobId) -> Option<IOop> {
        let Some(job) = self.job_state.get_mut(&ds_id) else {
            ...
            // lock when gathering job IDs to work on, it is possible to have a
            // out of date work list.
            warn!(self.log, "Job {ds_id} not on active list");
            return None;
        };
        ...
        if self.dependencies_need_cleanup() {
            match &mut out {
                ...
                | IOop::ExtentLiveNoOp { dependencies } => {
                    self.remove_dep_if_live_repair(dependencies, ds_id);
                }
            }
        }
            ...
            _ => {
                match job.work {
                    ...
                    IOop::Read {
                        ...
                        //
                        // Some errors cannot be handled and must be passed up
                        match e {
    ...
    /// Called to process a job which has already been acked to the upstairs
    fn handle_acked_job(
        &mut self,
        job: &DownstairsIO,
        read_data: &[ReadResponse],
        read_response_hashes: &[Option<u64>],
    ) -> Result<(), CrucibleError> {
        /*
        ...
        match &job.work {
            ...
            IOop::Read {
                ...
                if job.read_response_hashes != read_response_hashes {
                    ...
                    if job.replay {
                        ...
                        return Err(CrucibleError::GenericError(msg));
                    }
                }
            }
            /*
            ...
            | IOop::ExtentLiveNoOp { .. } => {
                return Err(CrucibleError::GenericError(format!(
                    "bad job received in handle_acked_job: {job:?}"
                )));
            }
        }
        Ok(())
    ...
    /// Handles a job that has not yet been acked
    ///
    /// Returns a boolean indication whether the guest should be notified, or an
    /// error if things have gone wrong.
    ///
    /// # Panics
    ...
    fn handle_job(
        ...
        let jobs_completed_ok = job_state_count.completed_ok();
        match &job.work {
            IOop::Read {
                dependencies: _dependencies,
                requests: _,
                ...
                if jobs_completed_ok == 1 {
                    ...
                    notify_guest = true;
                    assert_eq!(*ack, AckStatus::NotAcked);
                    *ack = AckStatus::AckReady;
                     ...
                     */
                    debug!(self.log, "Read already AckReady {}", job.ds_id);
                    if job.read_response_hashes != read_response_hashes {
                        // XXX This error needs to go to Nexus
                        // XXX This will become the "force all downstairs
                        // to stop and refuse to restart" mode.
            ...
            IOop::WriteUnwritten {
                ...
                if jobs_completed_ok == 2 {
                    notify_guest = true;
                    *ack = AckStatus::AckReady;
                    cdt::up__to__ds__write__unwritten__done!(|| job.guest_id);
                }
            }
            IOop::Flush {
                dependencies: _dependencies,
                ...
                if jobs_completed_ok == ack_at_num_jobs {
                    ...
                    *ack = AckStatus::AckReady;
                    ...
                    if deactivate {
                        debug!(self.log, "deactivate flush {} done", job.ds_id);
                    }
                }
                self.last_flush = job.ds_id;
            }
            ...
            IOop::ExtentFlushClose {
                ...
                if jobs_completed_ok == 3 {
                    notify_guest = true;
                    *ack = AckStatus::AckReady;
                    debug!(self.log, "ExtentFlushClose {} AckReady", job.ds_id);
                }
            }
            IOop::ExtentLiveRepair {
                dependencies: _,
                ...
                if jobs_completed_ok == 3 {
                    notify_guest = true;
                    debug!(self.log, "ExtentLiveRepair AckReady {}", job.ds_id);
                    *ack = AckStatus::AckReady;
                }
            }
            IOop::ExtentLiveReopen {
                ...
                if jobs_completed_ok == 3 {
                    notify_guest = true;
                    debug!(self.log, "ExtentLiveReopen AckReady {}", job.ds_id);
                    *ack = AckStatus::AckReady;
                }
            }
            IOop::ExtentLiveNoOp { dependencies: _ } => {
                ...
                if jobs_completed_ok == 3 {
                    notify_guest = true;
                    debug!(self.log, "ExtentLiveNoOp AckReady {}", job.ds_id);
                    *ack = AckStatus::AckReady;
                }
            }
        }
...
impl Upstairs {
     ...
     */
    fn ds_missing(&self, client_id: ClientId) {
        let mut client = self.clients[client_id].lock().unwrap();
        let new_state = match client.state {
            ...
            DsStateData::Migrating => DsStateData::Faulted,
            DsStateData::Faulted => DsStateData::Faulted,
            DsStateData::Deactivated => DsStateData::New,
            DsStateData::Repair => DsStateData::New,
            DsStateData::FailedRepair => DsStateData::New,
            DsStateData::LiveRepair { .. } => DsStateData::Faulted,
            DsStateData::LiveRepairReady => DsStateData::Faulted,
            DsStateData::Replacing => DsStateData::Replaced,
            _ => {
                /*
    ...
    async fn connect_region_set(
        ...
        match collate_status {
            Err(e) => {
                ...
                // downstairs, we have to verify that the downstairs are all in
            ...
            Ok(true) => {
                 ...
                 *
                 * As we repaired, downstairs should all be DsStateData::Repair.
                 *
...
/// Data-bearing variant of [`DsState`]
///
/// This type is used to implement richer state machines
#[allow(clippy::derive_partial_eq_without_eq)]
#[derive(Debug, Copy, Clone, PartialEq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "snake_case")]
pub(crate) enum DsStateData {
    New,
    BadVersion,
    WaitActive,
    WaitQuorum,
...
impl From<DsStateData> for DsState {
    fn from(d: DsStateData) -> DsState {
        match d {
            DsStateData::New => DsState::New,
            DsStateData::BadVersion => DsState::BadVersion,
            DsStateData::WaitActive => DsState::WaitActive,
            DsStateData::WaitQuorum => DsState::WaitQuorum,
            DsStateData::BadRegion => DsState::BadRegion,
            DsStateData::Disconnected => DsState::Disconnected,
            DsStateData::Repair => DsState::Repair,
            DsStateData::FailedRepair => DsState::FailedRepair,
            DsStateData::Active => DsState::Active,
            DsStateData::Faulted => DsState::Faulted,
            DsStateData::LiveRepairReady => DsState::LiveRepairReady,
            DsStateData::LiveRepair { .. } => DsState::LiveRepair,
            DsStateData::Migrating => DsState::Migrating,
            DsStateData::Offline => DsState::Offline,
            DsStateData::Replay => DsState::Replay,
            DsStateData::Deactivated => DsState::Deactivated,
            DsStateData::Disabled => DsState::Disabled,
            DsStateData::Replacing => DsState::Replacing,
            DsStateData::Replaced => DsState::Replaced,
        }
    }
}
...
impl AtomicIOState {
    ...
    /// Checks whether the job state for the given client matches
    ///
    /// Note that errors are not precisely checked, because we only store a
    /// single bit, not the full error.  In other words, if the job state is an
    /// error for the given client, any `state` argument that's an
    /// `IOState::Error(..)` will match it.
    fn client_state_matches(
        &self,
        client_id: ClientId,
        state: &IOState,
    ) -> bool {
