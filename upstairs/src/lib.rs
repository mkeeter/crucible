...
async fn process_message(
    ...
    let (upstairs_id, session_id, ds_id, result, extent_info) = match m {
        ...
        Message::ErrorReport {
            ...
            let _active = u.active.lock();
            let ds = u.downstairs.lock();
        ...
        + 'static,
{
    ...
    let (mut new_work, mut active_count) = {
        let mut ds = u.downstairs.lock();
        let new_work = ds.new_work(client_id);
    ...
    while let Some(new_id) = new_work.pop_first() {
        ...
        if u.lossy && random() && random() {
             ...
             */
            u.downstairs.lock().requeue_one(client_id, new_id);
            continue;
        }
         ...
         */
        let job = u.downstairs.lock().in_progress(new_id, client_id);
        if job.is_none() {
            continue;
        }
        ...
        match job.unwrap() {
            ...
            IOop::Flush {
                ...
                let my_extent_limit =
                    if u.ds_state.get(client_id) == DsState::LiveRepair {
                        extent_limit
                    } else {
                        None
                    };
        ...
        + 'static,
{
    ...
    // re_new.
    {
        let mut ds = up.downstairs.lock();
        let my_state = ds.ds_state.get(up_coms.client_id);
            ...
            f = fr.next() => {
                ...
                match f.transpose()? {
                    ...
                    Some(Message::RegionInfo { region_def }) => {
                        ...
                        match up.ds_state.get(up_coms.client_id) {
                            DsState::Offline => {
                                /*
        ...
        + 'static,
{
    ...
    };
    {
        let state = {
            let mut ds = up.downstairs.lock();
            let state = ds.ds_state.get(up_coms.client_id);
            if state == DsState::Replay {
                info!(
                    up.log,
    ...
    let pm_task = {
        ...
        tokio::spawn(async move {
            while let Some(m) = pm_task_rx.recv().await {
                 ...
                 */
                let my_state = up_c.ds_state.get(client_id);
            ...
            f = fr.next() => {
                ...
                match f.transpose()? {
                    ...
                    Some(Message::RepairAckId { repair_id }) => {
                        if up.downstairs.lock().rep_done(
...
async fn looper(
    ...
    'outer: loop {
        ...
        let target = {
            ...
            ds.ds_target[up_coms.client_id]
        };
        ...
        up.downstairs.lock().connected[up_coms.client_id] += 1;
...
/// Helper type to share `ClientData<DsState>` across tasks without locks
#[derive(Clone, Debug)]
struct SharedDsState(Arc<ClientData<AtomicDsState>>);
impl SharedDsState {
    fn new() -> Self {
        Self(Arc::new(ClientData([
            AtomicDsState::new(DsState::New),
            AtomicDsState::new(DsState::New),
            AtomicDsState::new(DsState::New),
        ])))
    }
    fn set(&self, i: ClientId, value: DsState) {
        self.0[i].store(value, std::sync::atomic::Ordering::Release)
    }
    fn get(&self, i: ClientId) -> DsState {
        self.0[i].load(std::sync::atomic::Ordering::Acquire)
    }
    fn iter(&self) -> impl Iterator<Item = DsState> {
        [
            self.get(ClientId::new(0)),
            self.get(ClientId::new(1)),
            self.get(ClientId::new(2)),
        ]
        .into_iter()
    }
}
...
impl Downstairs {
    ...
    fn live_repair_dep_check(
        ...
        if self.dependencies_need_cleanup(client_id) {
            self.remove_dep_if_live_repair(client_id, deps, ds_id)
        }
    }
    /**
    ...
    fn in_progress(
        ...
        match &mut out {
            ...
            | IOop::ExtentLiveNoOp { dependencies } => {
                self.live_repair_dep_check(client_id, dependencies, ds_id);
            }
            IOop::ExtentClose { .. } => {
                // This command should never exist on the upstairs side.
                // We only construct it for downstairs IO, after
    ...
    fn dependencies_need_cleanup(&mut self, client_id: ClientId) -> bool {
        self.ds_state.get(client_id) == DsState::LiveRepair
    ...
    fn remove_dep_if_live_repair(
        ...
        assert_eq!(self.ds_state.get(client_id), DsState::LiveRepair);
    ...
    fn repair_or_abort(&mut self) -> Result<()> {
        ...
        if not_ready > 0 {
            ...
            for i in ClientId::iter() {
                let s = self.ds_state.get(i);
                if s == DsState::Repair {
                    self.ds_state.set(i, DsState::FailedRepair);
    ...
    fn rep_in_progress(&mut self, client_id: ClientId) -> Option<Message> {
        if self.ds_state.get(client_id) != DsState::Repair {
            return None;
        }
     ...
     */
    fn enqueue(&mut self, mut io: DownstairsIO, ds_done_tx: mpsc::Sender<()>) {
        let mut skipped = 0;
        let is_write = matches!(io.work, IOop::Write { .. });
        for cid in ClientId::iter() {
            assert_eq!(io.state[cid], IOState::New);

            let current = self.ds_state.get(cid);
        ...
        if skipped == 3 {
            ...
            while ds_done_tx.try_send(()).is_err() {
                // TODO lol
            }
        } else if is_write {
            let mut handle = self.ds_active.get_mut(&ds_id).unwrap();
...
pub struct Upstairs {
     ...
     */
    active: FunSyncMutex<UpstairsState, 1>,
     ...
     */
    downstairs: FunSyncMutex<Downstairs, 0>,
     ...
     */
    flush_info: Arc<std::sync::Mutex<FlushInfo>>,
     ...
     */
    need_flush: Arc<std::sync::Mutex<bool>>,
...
#[derive(Debug)]
struct FunSyncMutex<T, const N: u64>(Arc<std::sync::Mutex<T>>);

impl<T, const N: u64> FunSyncMutex<T, N> {
...
impl Upstairs {
    ...
    pub fn new(
        ...
        Arc::new(Upstairs {
            active: FunSyncMutex(Arc::new(std::sync::Mutex::new(
                UpstairsState::default(),
            ))),
            uuid,
            session_id: Uuid::new_v4(),
            generation: Arc::new(std::sync::Mutex::new(gen)),
            guest,
            downstairs: FunSyncMutex(Arc::new(std::sync::Mutex::new(ds))),
            ds_state,
            flush_info: Arc::new(std::sync::Mutex::new(FlushInfo::new())),
            ddef: Arc::new(std::sync::Mutex::new(rd_status)),
            encryption_context,
            need_flush: Arc::new(std::sync::Mutex::new(false)),
            stats,
            lossy: opt.lossy,
    ...
    async fn set_deactivate(
        ...
        let mut active = self.active.lock();
        let mut ds = self.downstairs.lock();
        /*
    ...
    async fn is_deactivating(&self) -> bool {
        self.active.lock().up_state == UpState::Deactivating
    }
    ...
    async fn deactivate_transition_check(&self) {
        ...
        if active.up_state == UpState::Deactivating {
            ...
            for i in ClientId::iter() {
                let ds_state = ds.ds_state.get(i);
    ...
    async fn is_active_requested(&self) -> bool {
        self.active.lock().active_request
    }
    ...
    fn last_flush_id(&self, client_id: ClientId) -> JobId {
        let ds = self.downstairs.lock();
    ...
    async fn submit_flush(
         ...
         */
        let mut gw = self.guest.guest_work.lock().await;
        let mut downstairs = self.downstairs.lock();
    ...
    async fn ds_missing(&self, client_id: ClientId) {
        let ds = self.downstairs.lock();
        let current = ds.ds_state.get(client_id);
        ...
        ds.ds_state.set(client_id, new_state);
    }
    ...
    async fn _ds_is_replay(&self, client_id: ClientId) -> bool {
        let ds = self.downstairs.lock();
        // TODO: atomic transition here?
        if ds.ds_state.get(client_id) == DsState::Replay {
            info!(
                self.log,
                "[{}] {} Transition from Replay to Active",
                client_id,
            ...
            ds.ds_state.set(client_id, DsState::Active);
            return true;
        }
        false
    }
    ...
    fn ds_transition_with_lock(
        ...
        info!(
            ...
            self.uuid,
            self.session_id,
            ds.ds_state.get(ClientId::new(0)),
            ds.ds_state.get(ClientId::new(1)),
            ds.ds_state.get(ClientId::new(2)),
            new_state
        );

        let old_state = ds.ds_state.get(client_id);
    ...
    fn collate_downstairs(
        ...
        if let Some(reconcile_list) = reconcile_list {
            ...
            for i in ClientId::iter() {
                ...
                ds.ds_state.set(i, DsState::Repair);
            }
    ...
    async fn do_reconciliation(
        ...
        self.downstairs.lock().reconcile_current_work = None;
        Ok(())
    }
    ...
    async fn connect_region_set(
        ...
        match collate_status {
            Err(e) => {
                ...
                for i in ClientId::iter() {
                    let s = ds.ds_state.get(i);
                    if s == DsState::WaitQuorum {
                        ds.ds_state.set(i, DsState::FailedRepair);
                        warn!(
                            self.log,
            ...
            Ok(true) => {
                let repair_commands =
                    self.downstairs.lock().reconcile_task_list.len();
                ...
                let mut stats = self.stats.up_stat_wrap.lock().await;
                let mut active = self.active.lock();
                let ds = self.downstairs.lock();
                /*
                ...
                if ready != 3 {
                    ...
                    for i in ClientId::iter() {
                        ds.ds_state.set(i, DsState::Active);
                    }
                    active.set_active()?;
                    info!(
                        self.log,
         ...
         */
        assert!(self.downstairs.lock().reconcile_current_work.is_none());
    ...
    async fn ds_state_copy(&self) -> ClientData<DsState> {
        ClientData([
            self.ds_state.get(ClientId::new(0)),
            self.ds_state.get(ClientId::new(1)),
            self.ds_state.get(ClientId::new(2)),
        ])
    }
    ...
    async fn ds_work_active(&self) -> u32 {
        self.downstairs.lock().ds_active.len() as u32
    }
    ...
    async fn add_ds_region(
        ...
        if let Some(uuid) = ds.ds_uuid.get(&client_id) {
            if *uuid != client_ddef.uuid() {
                ...
                // okay.
                if ds.ds_state.get(client_id) == DsState::Replaced {
                    warn!(
                        self.log,
    ...
    async fn process_ds_operation(
         ...
         */
        let ds_state = ds.ds_state.get(client_id);
        match ds_state {
    ...
    async fn replace_downstairs(
        ...
        if new_client_id.is_some() {
            ...
            match ds.ds_state.get(new_client_id.unwrap()) {
                DsState::Replacing
                | DsState::Replaced
                | DsState::LiveRepairReady
                | DsState::LiveRepair => {
        ...
        for client_id in ClientId::iter() {
            ...
            match ds.ds_state.get(client_id) {
                DsState::Replacing
                | DsState::Replaced
                | DsState::LiveRepairReady
                | DsState::LiveRepair => {
        ...
        if ds.ds_set_faulted(old_client_id) {
            let _ = ds_done_tx.blocking_send(());
        }
...
async fn up_ds_listen(up: &Arc<Upstairs>, mut ds_done_rx: mpsc::Receiver<()>) {
    ...
    while ds_done_rx.recv().await.is_some() {
        ...
        let mut up_stat_lock = up.stats.up_stat_wrap.lock().await;
        let jobs_checked = {
            // Scope to properly drop `ds`
            let mut ds = up.downstairs.lock();
            ...
            for ds_id_done in ack_list.iter() {
                ...
                ds.cdt_gw_work_done(ds_id, gw_id, io_size, &mut up_stat_lock);
        ...
        for (gw_id, ds_id, data, result) in gw_complete {
            gw.gw_ds_complete(gw_id, ds_id, data, result, &up.log).await;
        }
        debug!(
            up.log,
...
async fn gone_too_long(up: &Arc<Upstairs>, ds_done_tx: mpsc::Sender<()>) {
    ...
    let up_state = up.active.lock().up_state;
...
async fn process_new_io(
    ...
    match req.op() {
        ...
        BlockOp::QueryGuestIOReady { data } => {
            *data.lock().await = up.guest_io_ready();
            req.send_ok();
        }
        ...
        BlockOp::QueryWorkQueue { data } => {
            ...
            let active_count = up
                ...
                .count();
            let ds_count = up.downstairs.lock().ds_active.len();
...
async fn show_all_work(up: &Arc<Upstairs>) -> WQCounts {
    ...
    let gw = up.guest.guest_work.lock().await;
    let ds = up.downstairs.lock();
    ...
    let up_done = gw.completed.to_vec();
 ...
 */
fn show_guest_work(gw: &GuestWork) -> usize {
