...
impl ActiveConnection {
    ...
    async fn proc_frame(
        ...
        let r = match m {
            ...
            Message::Flush {
                ...
                self.do_work_if_ready(job_id, new_flush, flags, dss, region)
                    .await?
            }
            ...
            Message::ReadRequest {
                ...
                self.do_work_if_ready(job_id, new_read, flags, dss, region)
                    .await?
            }
            ...
            Message::ExtentLiveClose {
                ...
                self.do_work_if_ready(job_id, ext_close, flags, dss, region)
                    .await?
            }
            Message::ExtentLiveFlushClose {
                job_id,
            ...
            Message::ExtentLiveRepair {
                ...
                self.do_work_if_ready(job_id, new_repair, flags, dss, region)
                    .await?
            }
            Message::ExtentLiveReopen {
                job_id,
                ...
                self.do_work_if_ready(job_id, new_open, flags, dss, region)
                    .await?
            }
            Message::ExtentLiveNoOp {
                job_id,
                ...
                self.do_work_if_ready(job_id, new_noop, flags, dss, region)
                    .await?
            }
    ...
    async fn do_work_for(
        ...
        while let Some(new_id) = new_work.pop_front() {
            ...
            if let Some(failed) =
                self.do_work(new_id, dw, flags, dss, region).await?
            {
    ...
    async fn do_work_if_ready(
        ...
        if self.work.check_ready(ds_id, &mut job) {
            ...
            let mut prev = self.do_work(ds_id, job, flags, dss, region).await?;
            while let Some(ds_id) = prev.take() {
                let job = self.work.in_progress(ds_id).unwrap();
                prev = self.do_work(ds_id, job, flags, dss, region).await?;
            }
            Ok(true)
        } else {
    ...
    async fn do_work(
        ...
        let m = self.do_work_inner(new_id, &job, flags, region).await;
        ...
        + 'static,
{
    ...
    let mut recv_task =
        tokio::spawn(recv_task(id, fr, handle, log.new(o!("task" => "recv"))));
            ...
            new_read = fr.next() => {
                match new_read {
                    None => {
                        warn!(
                            log,
                            "upstairs {id:?} disconnected; recv_task stopping"
                        );
                    ...
                    Some(Err(e)) => {
                        ...
                        bail!("Error reading from upstairs {id:?}: {}", e);
                    }
                    Some(Ok(msg)) => {
                        let m = DownstairsRequest::Message { id, msg };
                        if let Err(e) = handle.tx.send(m) {
                            bail!(
                                "Failed sending Message to handle for \
                                 upstairs {id:?}: {e:?}"
                            );
                        }
                    },
...
impl Downstairs {
    ...
    async fn do_work_for(&mut self, conn_id: ConnectionId) -> Result<()> {
        ...
        state
            .do_work_for(&self.flags, &mut self.dss, &mut self.region)
            .await
    }
    ...
    /// This is it: the function which does everything for the Downstairs
...
impl Work {
    ...
    ///
    /// Otherwise, the job remains in the active map.
    ///
    /// Readiness depends on outstanding dependencies.
    ///
    /// # Panics
    /// If the given `JobId` is not present in the active map
    fn in_progress(&mut self, ds_id: JobId) -> Option<IOop> {
        let Some(mut work) = self.active.remove(&ds_id) else {
