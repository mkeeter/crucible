...
/// Handle and metadata for a running Downstairs task
#[derive(Debug)]
pub struct DownstairsTaskHandle {
    pub task: tokio::task::JoinHandle<Result<()>>,
    pub addr: SocketAddr,
    pub repair_addr: SocketAddr,
}

impl DownstairsTaskHandle {
    /// Aborts the running Downstairs task
    pub fn abort(self) {
        self.task.abort()
    }
}
...
pub async fn start_downstairs(
    ...
    Ok(DownstairsTaskHandle {
        task: join_handle,
        addr: local_addr,
        repair_addr: repair_listener,
    })
}
...
pub async fn clone_region(
    ...
    let info = crucible_common::BuildInfo::default();
    let log = ds.log.new(o!("task" => "clone".to_string()));
    info!(log, "Crucible Version: {}", info);
    info!(
        log,
        "Upstairs <-> Downstairs Message Version: {}", CRUCIBLE_MESSAGE_VERSION
