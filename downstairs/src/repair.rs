...
mod test {
    use super::*;
    use crate::region::{extent_dir, extent_file_basename, extent_file_name};
    ...
    async fn extent_expected_files() -> Result<()> {
        ...
        ex_files.sort();
        let expected = vec!["001.db", "001.db-shm", "001.db-wal"];
    ...
    async fn extent_expected_files_short() -> Result<()> {
        ...
        rm_file.push(extent_file_basename(1));
    ...
    async fn extent_expected_files_fail() -> Result<()> {
        ...
        // Delete db
        let mut rm_file = extent_dir.clone();
        rm_file.push(extent_file_name(2, ExtentType::Db));
        std::fs::remove_file(&rm_file).unwrap();
